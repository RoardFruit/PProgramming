...
package object barneshut {
  ...
  extends Quad {
    ...
    val smass=bodies.map(_.mass).sum
    (smass,bodies.map(b=>b.mass*b.x).sum/smass,bodies.map(b=>b.mass*b.y).sum/smass)
  }
    ...
    def insert(b: Body): Quad = {
      if(size>minimumSize) {
        ...
        (bodies:+b).foldLeft(newFork)((fork,body)=>fork insert body)
      }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val bodyX=(b.x max boundaries.minX) min boundaries.maxX
      val bodyY=(b.y max boundaries.minY) min boundaries.maxY
      val x=((bodyX-boundaries.minX)/sectorSize).toInt min sectorPrecision-1
      val y=((bodyY-boundaries.minY)/sectorSize).toInt min sectorPrecision-1
      this(x,y)+=b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      (0 until matrix.length) foreach (i=>newSectorMatrix.matrix(i)=this.matrix(i) combine that.matrix(i))
      newSectorMatrix
    }
